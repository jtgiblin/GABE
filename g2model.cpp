...
{
    return profile(tin)*(PNorm*exp(-1.*(pw2(dx*(i-N/2.)) + pw2(dx*(j-N/2.)) + pw2(dx*(k-N/2.)))/(sigma2)));
...
{
    return profile(tin)*alpha*PNorm*exp(-1.*(pw2(dx*(i-N/2.)) + pw2(dx*(j-N/2.)) + pw2(dx*(k-N/2.)))/(sigma2));
}
...
gNum energyDensity(INDECIES_f,gNum tin)
{
    return - massGaussian1(INDECIES,tin);
}
