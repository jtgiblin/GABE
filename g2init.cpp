...
fftwl_complex *fkf, *fkd; //holds the generated initial conditions in momentum space for teh fiedld (fkf) and its derivative (fkd)
...
void randInit(long double f[][N][N],long double df[][N][N],long double d2vdf2)// function that implements the random (or pseudo-random initial conditions for the field values
{
...
#pragma omp parallel for private (j,k) num_threads (tot_num_thrds)
    LOOP
    {
        f[i][j][k] += inic[k + N*(j + N*i)];// this is the addition of the random initial conditions to that dictated in the model file
